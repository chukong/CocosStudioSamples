...
void CocosGUIExamplesMapScene::onEnter()
{
    ...
    exit_button->setPosition(ccp(m_pUILayer->getContentSize().width - exit_button->getContentSize().width, exit_button->getContentSize().height * 2));
    exit_button->addReleaseEvent(this, coco_releaseselector(CocosGUIExamplesMapScene::menuCloseCallback));
...
void CocosGUIExamplesMapScene::menuCloseCallback(CCObject* pSender)
{
...
void CocosGUIExamplesMapScene::buildTouchEvent(CCObject *pSender)
{
    ...
    alert_imageview->loadTexture(buildPngPaths[build_button->getWidgetTag() - MAP_BUTTON_TAG]);
